...
describe ActionController::Base, '#render', type: :controller do
  ...
  context 'supporting the bulk extension' do
    before(:each) do
    ...
    context 'given mixed error types' do
      let(:serialized_bulk_errors) do
        {
          'errors' => [
            {
      ...
      controller do
        def create
          user = User.new(name: 'lucas', email: 'lucas')
          user.valid?
          errors = [
            {
              detail: 'Name can\'t be blank',
          ...
          render jsonapi_errors: [user.errors, [errors]], extensions: ['bulk']
        end
    ...
    context 'given no errors for some resources' do
      let(:serialized_bulk_errors) do
        {
          'errors' => [
            {
      ...
      controller do
        def create
          ...
          render jsonapi_errors: [user.errors, nil, [], [errors]], extensions: ['bulk']
        end
    ...
    context 'when not rendering enough errors' do
      controller do
        def create
          user = User.new(name: 'lucas', email: 'lucas@')
          user.valid?

          render jsonapi_errors: user.errors, extensions: ['bulk']
        end
